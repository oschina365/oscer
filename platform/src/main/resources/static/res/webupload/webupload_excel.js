...
jQuery(function () {
        ...
        if (state === 'ready') {
            ...
            if (stats.uploadFailNum) {
                text = '已成功上传' + stats.successNum + '个文件，' +
                    stats.uploadFailNum + '个文件上传失败，<a class="retry" href="#">重新上传</a>失败文件或<a class="ignore" href="#">移除</a>'
            }
    ...
    function setState(val) {
        ...
        switch (state) {
            ...
            case 'finish':
                stats = uploader.getStats();
                if (stats.successNum) {
                    layer.msg("上传成功",{icon:"6",title:"上传文件"})
                } else {
                    // 没有成功的图片，重设
                    state = 'done';
                    location.reload();
    ...
    uploader.onError = function (code) {
        if (code == "Q_EXCEED_NUM_LIMIT") {
            layer.msg("最多一次性只能上传"+uploader.options.fileNumLimit+"个文件",{icon:"5",title:"选择文件"})
        }else if (code == "F_EXCEED_SIZE") {
            layer.msg("单个文件上传最大"+fileSingleSize+"M",{icon:"0",title:"选择文件"})
        }else if (code == "Q_EXCEED_SIZE_LIMIT") {
            layer.msg("已超出文件上传最大限制："+allFileSize+"M",{icon:"0",title:"选择文件"})
        }else{
    ...
    $info.on('click', '.ignore', function () {
        ...
        if(successFileNum==0){
            ...
            $(".statusBar>div.info").text("选中"+(successFileNum)+"个文件，共"+(fileSize)/1024/1024+"M。");
            $("#filePicker2").removeClass("element-invisible");
