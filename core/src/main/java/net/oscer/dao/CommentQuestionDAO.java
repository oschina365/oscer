...
public class CommentQuestionDAO extends CommonDao<CommentQuestion> {
    ...
    public List<CommentQuestion> allByUser(long user, long status) {
        ...
        String sql = "select MAX(`id`) as id from comment_questions where user=? and status =? GROUP BY question order by id desc";
        if (status > 0) {
            sql = "select MAX(`id`) as id from comment_questions where user=? and 1=? GROUP BY question order by id desc";
        }
